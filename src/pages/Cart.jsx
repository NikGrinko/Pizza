...
import { removeCart, removeItemCart, minusItemCart, plusItemCart } from '../redux/actions/cartActions';

const logoDiscription = 'Cамая реактивная пицца';
...
const Cart = () => {
    ...
    const minusItem = (index) => {
        dispatch(minusItemCart(index));
    }
    const plusItem = (index) => {
        dispatch(plusItemCart(index))
    }
    const paymantOrder = () => {
        alert('bingo')
    }
                    ...
                    <div className='cart-gallery'>
                        <div className='gallery-header'>
                            <div className='gallery-header__title'>
                                <div className="gallery-header__title-image"><svg width="31" height="31" viewBox="0 0 31 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button onClick={() => remove()} className="gallery-header__button">
                                ...
                                <span className='gallery-header__button-text'>Очистить корзину</span>
                            </button>
                        </div>
                        <ul className='gallery-list'>
                            {addedPizzas.map(item => {
                                return <CartItem key={item.id} item={item} globalItem={items} removeItem={removeItem} minusItem={minusItem} plusItem={plusItem} />
                            })}
                        </ul>
                        <div className='gallery-footer'>
                            <div className="gallery-footer__info">
                                <div className="current-pizza">Всего пицц: <span>{totalCount} шт.</span></div>
                                <div className='booking-amount'>Сумма заказа: <span>{totalPrice} ₽</span></div>
                            </div>
                            <div className='gallery-footer__buttons'>
                                <NavLink to='/' className="back-to-home"><svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M7 13L1 6.93015L6.86175 1" stroke="#D3D3D3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
                                    <span className='back-to-home__text'>Вернуться назад</span></NavLink>
                                <button onClick={paymantOrder} className='payment'>Оплатить сейчас</button>
                            </div>
                        </div>
                    </div>
                    :
